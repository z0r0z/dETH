...
contract dETHTest is Test, TestPlus {
    ...
    address constant defaultSender = 0x1804c8AB1F12E6bbf3894d4083f33e07309d1f38;
    ...
    address constant pointEval = 0x000000000000000000000000000000000000000A;
    ...
    function testFuzz_Deposit(uint256 amount) public {
        ...
        assertEq(
            deth.balanceOf(address(this)), depositAmount, "User dETH balance should match deposit"
        );
    }
    ...
    function testFuzz_PartialWithdraw(uint256 amount) public {
        ...
        // Check balances.
        assertEq(
            address(deth).balance,
            depositAmount - withdrawAmount,
            "Contract balance should be reduced"
        );
        assertEq(
            deth.balanceOf(address(this)),
            depositAmount - withdrawAmount,
            "User dETH should be reduced"
        );
        assertEq(
            address(this).balance,
            balanceBefore + withdrawAmount,
            "ETH should be partially returned"
        );
    }
    ...
    function testFuzz_TransferFrom(address spender, address recipient, uint256 amount) public {
        vm.assume(
            spender != address(0) && spender != address(this) && recipient != address(0)
                && spender != recipient
        );
        uint256 transferAmount = bound(amount, 1, 1000 ether);
    ...
    function testFailReverseIncorrectTransferId() public {
        ...
        // Try to reverse nonexisting transfer.
        vm.expectRevert(TransferFinalized.selector);
    ...
    function testFuzz_ReverseTransfer(address recipient, uint256 amount) public {
        ...
        assertEq(
            deth.balanceOf(address(this)),
            transferAmount,
            "Original sender should receive tokens back"
        );
    }

    function _depositAndTransfer(address to, uint256 amount) internal returns (bytes32) {
        bytes32 transferId = keccak256(abi.encodePacked(address(this), to, amount, block.timestamp));
    ...
    function testFuzz_WithdrawFrom(address spender, uint256 amount) public {
        vm.assume(
            spender != address(0) && spender != address(this) && spender.code.length == 0
                && spender != defaultSender && amount != 0 && spender != blake2f
        );
    ...
    function testFuzz_PartialWithdrawFrom(address spender, uint256 amount) public {
        ...
        assertEq(
            deth.allowance(address(this), spender),
            depositAmount - withdrawAmount,
            "Allowance should be reduced"
        );
    }

    function testFuzz_FailWithdrawFromInsufficientAllowance(address spender, uint256 amount)
        public
    {
    ...
    function testFailWithdrawFromZero(address spender) public {
        vm.assume(
            spender != address(0) && spender != create2deployer && spender != console
                && spender != pointEval
        );
